...
public class TagAliasOperatorHelper {
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        /*if (tagAliasBean.action == ACTION_DELETE) {
        ...
        if (jPushMessage.getErrorCode() == 0) {
            ...
            LogUtils.i(TAG, logs);
            if (tagAliasBean.action == ACTION_SET) {
                SPUtils.getInstance().put(Constants.AFTER_LOGIN_SET_JPUSH_ALIAS_SUCCESS, true);
            }
            // ToastUtils.showShort(logs);
        } else {
            String logs = "Failed to " + getActionStr(tagAliasBean.action) + " alias, errorCode:" + jPushMessage.getErrorCode();
            LogUtils.e(TAG, logs);
            if (ACTION_SET == tagAliasBean.action && !RetryActionIfNeeded(jPushMessage.getErrorCode(), tagAliasBean)) {
                if (BuildConfig.LOG_DEBUG) {
                    ToastUtils.showShort(logs);
                }
            }
