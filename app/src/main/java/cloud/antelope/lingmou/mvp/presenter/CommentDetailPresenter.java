...
public class CommentDetailPresenter extends BasePresenter<CommentDetailContract.Model, CommentDetailContract.View> {
    ...
    @Inject
    public CommentDetailPresenter(CommentDetailContract.Model model, CommentDetailContract.View rootView) {
    ...
    public void queryReplyPage(String columnId, String contentId, String replyId) {
        if (!NetworkUtils.isConnected()) {
            mRootView.showMessage(Utils.getContext().getResources().getString(R.string.network_disconnect));
            return;
        }

        mModel.queryReplyPage(columnId, contentId, replyId)
    ...
    public void addReply(String columnId, String contentId, String replyId, String topReplyId,
        ...
        mModel.addReply(columnId, contentId, topReplyId, upReplyId, reUserName, reUserId, reply)
                .subscribeOn(Schedulers.io())
                // .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(disposable -> {
                    mRootView.showLoading("");//显示下拉刷新的进度条
