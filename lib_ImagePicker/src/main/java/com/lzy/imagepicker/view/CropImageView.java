...
public class CropImageView extends AppCompatImageView {
    ...
    /******************************** 中间的FocusView绘图相关的参数 *****************************/
    public enum Style {
        RECTANGLE, CIRCLE, FACE
    }

    private Style[] styles = {Style.RECTANGLE, Style.CIRCLE, Style.FACE};

    private int mMaskColor = 0xAF000000;   //暗色
    private int mBorderColor = 0xAA808080; //焦点框的边框颜色
    private int mFaceColor = Color.parseColor("#090b0d");   //面部识别需要覆盖的颜色
    ...
    private int mScanEndPosition;
    private int mScanStartPosition;
    ...
    private void initImage() {
        ...
        if (null == mScanBitmap) {
            mScanBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.face_scan);
            int scanWidth = mScanBitmap.getWidth();
            int scanHeight = mScanBitmap.getHeight();
            int newScanWidth = viewWidth;
            int newScanHeight = newScanWidth * scanHeight / scanWidth;
            mScanBitmap = Bitmap.createScaledBitmap(mScanBitmap, newScanWidth, newScanHeight, false);
            mScanTop = mScanStartPosition = -newScanHeight + 20;
        }

        if (null == mFontBitmap) {
            mFontBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.camera_face_font);
            int width = mFontBitmap.getWidth();
            int height = mFontBitmap.getHeight();
            int newWidth = viewWidth;
            int newHeight = newWidth * height / width;
            mFontBitmap = Bitmap.createScaledBitmap(mFontBitmap, newWidth, newHeight, false);
            mScanEndPosition = newHeight - mScanBitmap.getHeight();
        }

        mOldFocusRect.left = mFocusMidPoint.x - mFocusWidth / 2;
        mOldFocusRect.right = mFocusMidPoint.x + mFocusWidth / 2;
        mOldFocusRect.top = mFocusMidPoint.y - mFocusHeight / 2;
        mOldFocusRect.bottom = mFocusMidPoint.y + mFocusHeight / 2;

        mFocusRect.left = 170;
        mFocusRect.right = viewWidth - 170;
        mFocusRect.top = 100;
        mFocusRect.bottom = mFontBitmap.getHeight() - 110;

        mFaceRect.left = 0;
        mFaceRect.top = 0;
        mFaceRect.right = mFontBitmap.getWidth();
        mFaceRect.bottom = mFontBitmap.getHeight();

        mDstFaceRect.left = 0;
        mDstFaceRect.right = viewWidth;
        mDstFaceRect.top = 0;
        mDstFaceRect.bottom = mFontBitmap.getHeight();

        mScanRect.left = 0;
        mScanRect.top = 0;
        mScanRect.right = mScanBitmap.getWidth();
        mScanRect.bottom = mScanBitmap.getHeight();

        mDstScanRect.left = 0;
        mDstScanRect.right = viewWidth;
        mDstScanRect.top = 30;
        mDstScanRect.bottom = mScanBitmap.getHeight() + 30;

        mFaceRectF.left = 0;
        mFaceRectF.top = 0;
        mFaceRectF.right = mFontBitmap.getWidth();
        mFaceRectF.bottom = mFontBitmap.getHeight();


        //适配焦点框的缩放比例（图片的最小边不小于焦点框的最小边）
//        float fitFocusScale = getScale(mImageWidth, mImageHeight, mFocusWidth, mFocusHeight, true);
        float fitFocusScale = getScale(mImageWidth, mImageHeight, mFontBitmap.getWidth(), mFontBitmap.getHeight(), true);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Style.RECTANGLE == mStyle) {
            mFocusPath.addRect(mOldFocusRect, Path.Direction.CCW);
            canvas.save();
            ...
            float radius = Math.min((mOldFocusRect.right - mOldFocusRect.left) / 2, (mOldFocusRect.bottom - mOldFocusRect.top) / 2);
            ...
            canvas.drawColor(mMaskColor);
            canvas.restore();
        } else if (Style.FACE == mStyle) {
            mFocusPath.addRect(mFaceRectF, Path.Direction.CCW);
            canvas.save();
            canvas.clipRect(0, 0, getWidth(), getHeight());
//            Log.i("cxm", "faceRect width =  " + mFaceRect.width() + "-- height = " + mFaceRect.height() + ",viewwidth =" + getWidth() + ",viewheight=" + getHeight());
            canvas.clipPath(mFocusPath, Region.Op.DIFFERENCE);
            canvas.drawColor(mFaceColor);
            canvas.restore();
        }

        if (mStyle == Style.FACE) {
            if (mIsScan) {
                canvas.drawBitmap(mScanBitmap, 0, mScanTop, mBorderPaint);
            }
            canvas.drawBitmap(mFontBitmap, mFaceRect, mDstFaceRect, mBorderPaint);
//            mBorderPaint.setColor(mFaceColor);
//            mBorderPaint.setColor(mMaskColor);
//            canvas.drawRect(0, mFontBitmap.getHeight(), getWidth(), getHeight(), mBorderPaint);
        } else {
    ...
    //开始扫描
    public void scanFace(boolean isScan) {
        mIsScan = isScan;
        if (isScan) {
            mFaceRecogHandler.sendEmptyMessage(1);
        } else {
            mFaceRecogHandler.removeMessages(1);
        }
    }

    private Handler mFaceRecogHandler = new Handler(){
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            mScanTop += 25;
            if (mScanTop >= mScanEndPosition) {
                mScanTop = mScanStartPosition;
            }
            invalidate();
            mFaceRecogHandler.sendEmptyMessageDelayed(1, 10);
        }
    };
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mSaving || null == getDrawable() || mIsScan) {
...
//        float minScale = getScale(mRotatedImageWidth, mRotatedImageHeight, mFocusWidth, mFocusHeight, true);
        float minScale = getScale(mRotatedImageWidth, mRotatedImageHeight, mFontBitmap.getWidth(), mFontBitmap.getHeight(), true);
...
//        Log.v("cxm", "fixTranslation-- imageRect.left = " + imageRect.left + ", imageRect.top=" + imageRect.top);
        if (mStyle == Style.FACE) {
            ...
            if (imageRect.left > mOldFocusRect.left) {
                deltaX = -imageRect.left + mOldFocusRect.left;
            } else if (imageRect.right < mOldFocusRect.right) {
                deltaX = -imageRect.right + mOldFocusRect.right;
            }
            if (imageRect.top > mOldFocusRect.top) {
                deltaY = -imageRect.top + mOldFocusRect.top;
            } else if (imageRect.bottom < mOldFocusRect.bottom) {
                deltaY = -imageRect.bottom + mOldFocusRect.bottom;
            }
        }
...
//        float minScale = getScale(mRotatedImageWidth, mRotatedImageHeight, mFocusWidth, mFocusHeight, true);
        float minScale = getScale(mRotatedImageWidth, mRotatedImageHeight, mFaceRect.width(), mFaceRect.height(), true);
    ...
    public Bitmap getCropBitmap(int expectWidth, int exceptHeight, boolean isSaveRectangle) {
        ...
        if (mStyle == Style.FACE) {
            ...
            return makeCropBitmap(srcBitmap, mOldFocusRect, getImageMatrixRect(), expectWidth, exceptHeight, isSaveRectangle);
        }
    }
    ...
    public void saveBitmapToFile(File folder, int expectWidth, int exceptHeight, boolean isSaveRectangle) {
        ...
        mSaving = true;
        if (mStyle == Style.FACE) {
            expectWidth = (int) mFocusRect.width();
            exceptHeight = (int) mFocusRect.height();
        }
