...
public class ImageRecyclerAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    private class ImageViewHolder extends ViewHolder {
        ...
        void bind(final int position) {
                ...
                public void onClick(View v) {
                    ...
                    if (mimeType.startsWith("video")) {
                        ...
                        if (cbCheck.isChecked() && mSelectedVideos.size() >= videoLimit) {
                            ToastUtils.showShort(R.string.video_limit, videoLimit);
                            cbCheck.setChecked(false);
                            mask.setVisibility(View.GONE);
                        } else {
                            if (cbCheck.isChecked() && mSelectedImages.size() >= mediaLimit) {
                                ToastUtils.showShort(R.string.select_limit, mediaLimit);
                                cbCheck.setChecked(false);
                                mask.setVisibility(View.GONE);
                            } else {
            ...
            imagePicker.getImageLoader().displayImage(mActivity, mediaItem.path, ivThumb, mImageSize, mImageSize, null); //显示图片
        }
